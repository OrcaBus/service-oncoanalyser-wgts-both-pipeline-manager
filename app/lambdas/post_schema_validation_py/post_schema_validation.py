...
def validate_inputs(
    ...
    # Get the input list
    inputs_list: List[Tuple[str, List[str]]] = [
        ("tumorDnaInputs", TUMOR_DNA_INPUTS),
        ("normalDnaInputs", NORMAL_DNA_INPUTS),
        ("tumorRnaInputs", TUMOR_RNA_INPUTS),
    ]

    for inputs_top_key_iter_, input_list_iter_ in inputs_list:
        nested_inputs_dict = inputs.get(inputs_top_key_iter_, {})
        for input_iter_key in input_list_iter_:
            data_uris.append(nested_inputs_dict.get(input_iter_key, None))
    ...
    data_uris = list(filter(
        lambda uri_iter_: (
            ...
            #   * belong to the ref-data bucket
            #   * file that belongs to the project prefix (tested from filemanager above)
            not (
